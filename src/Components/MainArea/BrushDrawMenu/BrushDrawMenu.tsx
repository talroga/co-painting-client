import { useEffect, useState } from "react";
import "./BrushDrawMenu.css";
import { Color } from "../../../Utils/Types/Color";
import { Point } from "../../../Utils/Types/Draw";
import { HexColorPicker } from "react-colorful";
import { DrawEventHandler } from "../CoPainting/CoPaintingLogic";
import { draw, parseAction } from "../../../Utils/Types/DrawActionFactory";
import { CoPaintingWebSocket } from "../../../Utils/CoPaintingWebSocket/CoPaintingWebSocket";

interface BrushDrawMenuProperties {
    canvas: HTMLCanvasElement,
    coPaintingWebsocket: CoPaintingWebSocket | undefined
}

function getMousePos(canvas: HTMLCanvasElement, event: MouseEvent): Point {
    var rect = canvas.getBoundingClientRect()
    const point = {
        x: event.clientX - rect.left,
        y: event.clientY - rect.top
      }
    console.log(point)
    return point
}

function BrushDrawMenu(props: BrushDrawMenuProperties): JSX.Element {
    const [radius, setRadius] = useState(8)
    const [color, setColor] = useState('#2DE39F')
    const [isDrawing, setIsDrawing] = useState<boolean>(false)
    console.log('Rerendering BrushMenuDraw, isDrawing: ' + isDrawing)

    useEffect(() => {
        const handleMouseDown = (event: MouseEvent) => {
            console.log('Drawing')
            setIsDrawing(true)
        }
        props.canvas.addEventListener('mousedown', handleMouseDown)

        return () => {
            props.canvas.removeEventListener('mousedown', handleMouseDown)
        }

    }, [props])

    useEffect(() => {
        const handleMouseUp = (event: MouseEvent) => {
            console.log('Not drawing')
            setIsDrawing(false)
        }
        props.canvas.addEventListener('mouseup', handleMouseUp)

        return () => {
            props.canvas.removeEventListener('mouseup', handleMouseUp)
        }

    }, [props])

    useEffect(() => {
        const handleMouseMove = (event: MouseEvent) => {
            console.log(isDrawing)
            if(isDrawing && props.coPaintingWebsocket !== undefined) {
                props.coPaintingWebsocket.sendActionData({type: 'Circle', data: {radius: radius, color: color, center: getMousePos(props.canvas, event)}})
            }
        }
        props.canvas.addEventListener('mousemove', handleMouseMove)
        return () => {
            props.canvas.removeEventListener('mousemove', handleMouseMove)
        }
    }, [radius, color, props, isDrawing])

    function onRadiusChange(e: React.FormEvent<HTMLInputElement>){
        let target = e.target as HTMLInputElement;
        const re = /^[0-9\b]+$/;
    
        // if value is not blank, then test the regex

        console.log(e.target)
    
        if (target.value === '' || re.test(target.value)) {
           setRadius(Number(target.value))
        }
        
    }

    return (
        <div className="BrushDrawMenu">
            <input type="text" value={radius} onChange={onRadiusChange}/>
            <HexColorPicker color={color} onChange={setColor}/>
        </div>
    );
}

export default BrushDrawMenu;
